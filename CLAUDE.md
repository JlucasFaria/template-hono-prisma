...
- **RefreshToken**: id, token (unique, 40 random bytes), userId (FK → User, cascade delete), expiresAt, createdAt
...
Structured JSON logs integrate with log aggregation platforms (Datadog, CloudWatch, Grafana Loki) and make querying and alerting much easier than plain text logs.
